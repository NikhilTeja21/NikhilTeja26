...
page_name = s.set_page_config(page_title="My Introduction",page_icon=":star2:",layout="wide",menu_items={})
s.title("Web Development")
s.header("Introduction")
"""
    - This is a Demo Website
    - This will not be furthur developed
...
s.time_input(label="Time",key="Key",help="This generates a ? mark at the topo right to know what we've prvided.",kwargs="WidgetKwargs")
""" - We can show the time"""
s.balloons()
""" - Interesting things"""
s.bar_chart(data=[2,3,4,5,4.9,9.73,2.34,6.75],width=10,height=500)
""" - We can draw bar graphs by providing the data"""
#s.camera_input(label="Click a photo",key=3,on_change='600x200.png')
s.checkbox(label="I agree to the terms and conditions",value=False,help="Please read the above data properly")
""" 
...
"""- This is incomplete I'll check this once completely"""
with s.container() :
    left, middle, right = s.columns(3)
    with left :
        s.number_input(label="Enter your Roll Number : ",min_value=0,value=100000000,step=1)
    with middle :
        ""
    with right :
